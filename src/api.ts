...
import { texts, PlatformAPI, OnServerEventCallback, LoginResult, Paginated, Thread, Message, CurrentUser, InboxName, MessageContent, PaginationArg, MessageSendOptions, SerializedSession, ServerEventType, ActivityType, MessageID, TextAttributes, TextEntity } from '@textshq/platform-sdk'
...
export default class OpenAI implements PlatformAPI {
  ...
  login = async ({ cookieJarJSON, jsCodeResult }): Promise<LoginResult> => {
    ...
    this.userAgent = jsCodeResult?.ua || texts.constants.USER_AGENT
  ...
  private setupChatGPT = async () => {
    ...
    if (texts.isLoggingEnabled) {
      texts.log('Setting up ChatGPT', conf)
    }

    this.chatgpt = new ChatGPTAPI(conf)
